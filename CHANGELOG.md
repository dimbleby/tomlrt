...
- `load()` now reports malformed UTF-8 as `TOMLParseError`.
...
- Replacing a value inside an inline table no longer moves its key or removes
  its comments.
- Custom mappings that repeat a key now consistently keep its last value,
  including when assigned inside arrays or tables.
- Copying or building documents keeps empty arrays of tables usable as arrays
...
- Reduce import overhead by using plain slotted records.
...
- Dumping a `[section]`-backed `Table` no longer drops its own comments and
  spacing.
...
- Added `leading_block` for multiline array and inline-table elements.
...
- Improved the performance and formatting consistency of section and
  array-of-tables mutations in large documents.
