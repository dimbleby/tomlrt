...
- **`_synth.py`** — plans Python mappings in input order, then emits
  ...
  Serialization consumes synthesized or extracted slot runs directly;
  only document construction builds logical views over them.
- **`_layout_ops.py`** — section-side mutation primitives: insert
  ...
  `reposition_install` brackets caller-owned installation. Inline replacements
  prepare their value before entering it, so conversion precedes deletion.
...
- **Container shape** is named explicitly by the `_Kind` enum in
  ...
  `_host`; dispatch on `_kind` rather than re-discovering the
  discriminator.
